...
import sys
sys.path.append('..')
from models import ResCNN18
from utils import Logger
from loss import LGMLoss
...
sys.stdout = Logger(os.path.join(args.save_dir, 'resnet18_lgm_new' + '.txt'))
...
if args.cuda:
    model.cuda()
criterion = LGMLoss(10, 512, margin=0.5, lambda_=0.01).cuda()
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        feat = model(data)
        loss, logits, logits_with_margin, cls_loss, gmm_loss = criterion(feat, target)
        loss.backward()
        optimizer.step()
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.1f}%)]\tLoss: {:.6f}\tLoss_cls: {:.6f}\tLoss_gmm: {:.6f}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                       100. * batch_idx / len(train_loader), loss.item(), cls_loss.item(), gmm_loss.item()))

    torch.save(model.state_dict(), '../checkpoint/resnet18/resnet18_lgm_cnn.pth')
    torch.save(criterion.state_dict(), '../checkpoint/resnet18/resnet18_lgm_crit.pth')
...
def inference():
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            feat = model(data)
            loss, output = criterion(feat, target)[:2]
            test_loss += loss.item()
