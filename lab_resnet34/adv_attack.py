...
class OursLoss(nn.Module):
    ...
    def forward(self, w, output, target):
        ...
        criterion_weight = WeightLoss(self.delta)
        criterion_l2 = L2Loss(self.delta)

        loss_l2 = criterion_l2(output, target) #intra
        loss_w = criterion_weight(w) #inter

        loss = loss_l2 + self.alpha * loss_w

        return loss

def ours_adv(img, label, model, criterion=OursLoss(), eps=0.031, iters=20, step=0.003, target_setting=False):
    adv = img.clone().detach()
    adv.requires_grad = True
    if adv.grad is not None:
        adv.grad.data.zero_()
