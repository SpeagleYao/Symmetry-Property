...
sys.stdout = Logger(os.path.join(args.save_dir, 'vgg16_pcl' + '.txt'))
...
if args.cuda:
    model.cuda()
filename = '../checkpoint/vgg16/vgg16_vallina.pth'
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss = loss_xent + loss_prox_512 - loss_conprox_512  # total loss
        ...
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.1f}%)]\tLoss: {:.6f}\tLoss_xce: {:.6f}'
                  '\tLoss_prox512: {:.6f}\tLoss_conprox512: {:.6f}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                       100. * batch_idx / len(train_loader),
                       loss.item(),loss_xent.item(),loss_prox_512.item(),
                       loss_conprox_512.item()))

    torch.save(model.state_dict(), '../checkpoint/vgg16/vgg16_pcl.pth')
...
for epoch in range(1, args.epochs + 1):
    if epoch in [args.epochs * 0.5, args.epochs * 0.7, args.epochs * 0.9]:
