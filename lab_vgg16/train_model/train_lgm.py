...
sys.stdout = Logger(os.path.join(args.save_dir, 'vgg16_lgm_new' + '.txt'))
...
if args.cuda:
    model.cuda()
criterion = LGMLoss(10, 512, margin=0.05, lambda_=0.01).cuda()
...
def train(epoch):
    ...
    torch.save(model.state_dict(), '../checkpoint/vgg16/vgg16_lgm_cnn.pth')
    torch.save(criterion.state_dict(), '../checkpoint/vgg16/vgg16_lgm_crit.pth')
