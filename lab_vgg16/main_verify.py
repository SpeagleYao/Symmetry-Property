...
parser.add_argument('--data-dir', type=str, default='data', metavar='N',
                help='')
parser.add_argument('--model-checkpoint', type=str, default='../cp_ablation/res34_a0.05_d0.0.pth', metavar='N')
...
def craft_adv_samples(data_loader, model, args, attack_method):
    ...
    if attack_method == 'c&w':
        bar = progressbar.ProgressBar(max_value=CW_EPOCHS)
    else:
        bar = progressbar.ProgressBar(max_value=10000//args.test_batch_size+1)
    for bi, batch in enumerate(data_loader):
        if attack_method == 'c&w' and bi >= CW_EPOCHS: break
...
def main():
    ...
    for attack_method in ['fgsm', 'pgd10', 'pgd20', 'pgd50', 'mim', 'c&w']: # 'pgd20', 'mim'
